...
export const Modal = ({ onClose, onFinanceAdded }) => {
  ...
  // Fetch categories da API ao montar o componente
  useEffect(() => {
    setIsVisible(true);
    const fetchCategories = async () => {
      try {
        const response = await categoriesCall(); // Chamada à API
  ...
  const mutation = useMutation((newFinance) => onCallAddFinance(newFinance), {
    ...
    onSuccess: () => {
      ...
      onClose();
      onFinanceAdded(); // Notifica a HomeScreen para atualizar os lançamentos
    },
  });
      ...
      >
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className="mb-8">
            ...
            <Select
              id="category"
              options={categoryOptions} // Categorias da API
              placeholder="Selecione uma categoria"
              value={formik.values.category}
              onChange={(option) => formik.setFieldValue("category", option)}
              styles={customStyles} // Estilização customizada
            />
