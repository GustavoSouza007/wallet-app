...
export const HomeScreen = () => {
  ...
  const loadFinancesData = async (date) => {
    ...
    const receitas = sortedData.filter((item) => Number(item.value) > 0);
    const despesas = sortedData.filter((item) => Number(item.value) < 0);
  ...
  const handleDeleteItem = async (id) => {
    try {
      ...
      setFinancesData((prevData) => {
        ...
        const receitas = updatedData.filter((item) => Number(item.value) > 0);
        const despesas = updatedData.filter((item) => Number(item.value) < 0);
